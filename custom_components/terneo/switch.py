...
SWITCH_REQUIRED_PARAMS = {
    "power": ParamNum.POWER_OFF,
    "children_lock": ParamNum.CHILDREN_LOCK,
    "cooling_mode": ParamNum.COOLING_CONTROL_WAY,
    "pre_control": ParamNum.PRE_CONTROL,
    "use_night_brightness": ParamNum.USE_NIGHT_BRIGHT,
    "window_open_control": ParamNum.WINDOW_OPEN_CONTROL,
    "nc_contact_control": ParamNum.NC_CONTACT_CONTROL,
    "lan_block": ParamNum.LAN_BLOCK,
    "cloud_block": ParamNum.CLOUD_BLOCK,
}
...
async def async_setup_entry(
    ...
    for description in SWITCH_DESCRIPTIONS:
        required = SWITCH_REQUIRED_PARAMS.get(description.key)
...
class TerneoSwitchEntity(CoordinatorEntity, SwitchEntity):
    ...
    def __init__(
        ...
        if description.key not in {"power", "cooling_mode", "children_lock"}:
    ...
    async def async_turn_on(self, **kwargs: Any) -> None:
        ...
        await async_execute_command(
            ...
            self._thermostat,
            action=f"turn on {self.entity_description.name or self.entity_description.key}",
        )
    ...
    async def async_turn_off(self, **kwargs: Any) -> None:
        ...
        await async_execute_command(
            ...
            self._thermostat,
            action=f"turn off {self.entity_description.name or self.entity_description.key}",
        )
        await self.coordinator.async_request_refresh()
